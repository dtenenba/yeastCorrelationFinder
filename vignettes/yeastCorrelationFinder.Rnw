...
%\VignettePackage{yeastCorrelationFinder}
...
\newcommand{\yeastCorrelationFinder}{\Rpackage{yeastCorrelationFinder}}
...
<<correlationFinder>>=
library(yeastCorrelationFinder)
tbl <- correlationFinder()
head(tbl)
@
...
\item Allocco, DJ  Kohane IS, Butte AJ. Quantifying the relationship between co-expression, co-regulation and gene function,
